...
LV_ATTRIBUTE_FAST_MEM static void draw_bg(const lv_area_t *coords, const lv_area_t *clip_area,
    ...
    if (mask_any) {
        for (h = draw_area.y1; h <= draw_area.y2; h++) {
            ...
            if (grad_dir == LV_GRAD_DIR_NONE) {
                _lv_blend_fill(clip_area, &blend_area, dsc->bg_color, mask_buf, mask_res, LV_OPA_COVER,
                               dsc->blend_mode);
    ...
    if (!mask_any && grad_dir == LV_GRAD_DIR_NONE) {
        ...
        _lv_blend_fill(clip_area, &blend_area, dsc->bg_color, mask_buf, LV_DRAW_MASK_RES_FULL_COVER, opa,
                       dsc->blend_mode);
    }
    ...
    if (_lv_area_intersect(&clip_area_sub, &blend_area, clip) && !_lv_area_is_in(&clip_area_sub, &bg_area, r_bg)) {
        ...
        if (w > 0) {
            ...
            for (y = clip_area_sub.y1; y <= clip_area_sub.y2; y++) {
                ...
                _lv_blend_fill(&clip_area_sub, &blend_area, dsc->shadow_color, *mask_act, mask_res, dsc->shadow_opa,
                               dsc->blend_mode);
        ...
        if (w > 0) {
            ...
            for (y = clip_area_sub.y1; y <= clip_area_sub.y2; y++) {
                ...
                if (!simple_sub) {
                    ...
                    mask_res = lv_draw_mask_apply(mask_buf, clip_area_sub.x1, y, w);
                    if (mask_res == LV_DRAW_MASK_RES_FULL_COVER) mask_res = LV_DRAW_MASK_RES_CHANGED;
                    _lv_blend_fill(&clip_area_sub, &blend_area, dsc->shadow_color, mask_buf, mask_res, dsc->shadow_opa,
                                   dsc->blend_mode);
                } else {
                    lv_opa_t line_opa = opa == LV_OPA_COVER ? sh_buf_tmp[0] : (sh_buf_tmp[0] * dsc->shadow_opa) >> 8;
                    _lv_blend_fill(&clip_area_sub, &blend_area, dsc->shadow_color, NULL, LV_DRAW_MASK_RES_FULL_COVER,
                                   line_opa,
                                   dsc->blend_mode);
                }
                sh_buf_tmp += corner_size;
