...
LV_ATTRIBUTE_FAST_MEM static void lv_draw_map(const lv_area_t *map_area, const lv_area_t *clip_area,
    ...
    if (!mask_any && draw_dsc->angle == 0 && draw_dsc->zoom == LV_IMG_ZOOM_NONE &&
        ...
        if (!mask_any && !transform && !chroma_key && draw_dsc->recolor_opa == LV_OPA_TRANSP && alpha_byte) {
            ...
            uint32_t mask_buf_size =
                    lv_area_get_size(&draw_area) > (uint32_t) hor_res ? hor_res : lv_area_get_size(&draw_area);
            ...
            for (y = 0; y < draw_area_h; y++) {
                ...
                if (px_i + draw_area_w < mask_buf_size) {
                    ...
                    _lv_blend_map(clip_area, &blend_area, map2, mask_buf, LV_DRAW_MASK_RES_CHANGED, draw_dsc->opa,
                                  draw_dsc->blend_mode);
            ...
            for (y = 0; y < draw_area_h; y++) {
                ...
                if (px_i + draw_area_w < mask_buf_size) {
                    ...
                    _lv_blend_map(clip_area, &blend_area, map2, mask_buf, mask_res, draw_dsc->opa,
                                  draw_dsc->blend_mode);
